...
async def create_invoice(payload: Invoice, db: Session = Depends(get_db)):
...
async def read_invoice(invoice_id: UUID, db: Session = Depends(get_db)):
...
async def delete_invoices(db: Session = Depends(get_db)):
