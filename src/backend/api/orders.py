...
async def create_orders(token: Annotated[str, Depends(is_token_valid)], payload: Order, db: Session = Depends(get_db)):
...
async def read_order(token: Annotated[str, Depends(is_token_valid)], order_id: UUID, db: Session = Depends(get_db)):
    order = crud.get_order(order_id, db)
    return order

@router.delete("/orders/{order_id}/", status_code=200)
async def delete_order(token: Annotated[str, Depends(is_token_valid)], order_id: UUID, db: Session = Depends(get_db)):
    order = crud.delete_order(order_id, db)
    return order

@router.get("/orders/", status_code=200)
async def read_orders(token: Annotated[str, Depends(is_token_valid)], db: Session = Depends(get_db)):
    orders = crud.get_orders(db)
    return orders

@router.delete("/orders/", status_code=200)
async def delete_orders(token: Annotated[str, Depends(is_token_valid)], db: Session = Depends(get_db)):
