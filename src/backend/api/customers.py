...
async def create_customer(token: Annotated[str, Depends(is_token_valid)], payload: Customer, db: Session = Depends(get_db)):
...
async def read_customer(token: Annotated[str, Depends(is_token_valid)], customer_id: UUID, db: Session = Depends(get_db)):
    customer = crud.get_customer(customer_id, db)
    return customer

@router.delete("/customers/{customer_id}/", status_code=200)
async def delete_customer(token: Annotated[str, Depends(is_token_valid)], customer_id: UUID, db: Session = Depends(get_db)):
    customer = crud.delete_customer(customer_id, db)
    return customer

@router.get("/customers/", status_code=200)
async def read_customers(token: Annotated[str, Depends(is_token_valid)], db: Session = Depends(get_db)):
    customers = crud.get_customers(db)
    return customers

@router.delete("/customers/", status_code=200)
async def delete_customers(token: Annotated[str, Depends(is_token_valid)], db: Session = Depends(get_db)):
