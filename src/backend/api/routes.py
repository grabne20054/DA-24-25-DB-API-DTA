...
async def create_route(payload: Route, db: Session = Depends(get_db)):
...
async def read_route(route_id: UUID, db: Session = Depends(get_db)):
    route = crud.get_route(route_id, db)
    return route

@router.delete("/routes/{route_id}/", status_code=200)
async def delete_route(route_id: UUID, db: Session = Depends(get_db)):
...
async def delete_routes(db: Session = Depends(get_db)):
