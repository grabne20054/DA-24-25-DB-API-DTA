...
async def create_address(token: Annotated[str, Depends(is_token_valid)], payload: Address, db: Session = Depends(get_db)):
...
async def read_address(token: Annotated[str, Depends(is_token_valid)], address_id: UUID, db: Session = Depends(get_db)):
    address = crud.get_address(address_id, db)
    return address

@router.delete("/addresses/{address_id}/", status_code=200)
async def delete_address(token: Annotated[str, Depends(is_token_valid)], address_id: UUID, db: Session = Depends(get_db)):
    address_id = crud.delete_address(address_id, db)
    return address_id

@router.get("/addresses/", status_code=200)
async def read_addresses(token: Annotated[str, Depends(is_token_valid)], db: Session = Depends(get_db)):
    addresses = crud.get_addresses(db)
    return addresses

@router.delete("/addresses/", status_code=200)
async def delete_addresses(token: Annotated[str, Depends(is_token_valid)], db: Session = Depends(get_db)):
